...
const Orders = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/v1/order/admin/getall`,
        {
          headers: {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      const token = localStorage.getItem('accessToken')
      await axios.put(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/v1/order/admin/update/${orderId}`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const updatePaymentStatus = async (orderId, newPaymentStatus) => {
    try {
      const token = localStorage.getItem('accessToken')
      await axios.put(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/v1/order/admin/payment/${orderId}`,
  ...
  const handleDeleteOrder = async (orderId) => {
    if (
      ...
      try {
        const token = localStorage.getItem('accessToken')
        await axios.delete(
          `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/v1/order/admin/delete/${orderId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
